...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[tg-my-reputation] verify', debug);
    ...
    const reputation = (rows || []).reduce((sum, r: any) => sum + (r?.value || 0), 0);

    return new Response(JSON.stringify({ reputation, history: rows || [] }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (e) {
    console.error('tg-my-reputation error:', e);
